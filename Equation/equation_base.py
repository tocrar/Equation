...
def equation_extend():
    ...
    def first_of_list(args):
        return args[0]

    addFn('last', "last({0:s})", "\\text{last}\\left({0:s}\\right)", '+', last_of_list)
    addFn('first', "first({0:s})", "\\text{first}\\left({0:s}\\right)", '+', first_of_list)
    ...
    if has_numpy:
        ...
        addFn('log', "log({0:s})", "\\log{{{0:s}}}", '+', np.log)
        ...
        addFn('log', "log({0:s})", "\\log{{{0:s}}}", '+', math.log)
